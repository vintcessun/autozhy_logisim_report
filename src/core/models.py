...
class TaskRecord(BaseModel):
    ...
    task_instruction_docs: List[str] = []  # 当前任务关联的指导/数据文件（优先用于验证）

    # 扩展字段：实验目的与环境
    experiment_objective: str = ""
    experiment_environment: str = ""
    thinking_questions: List[str] = []
    problem_answers: List[dict[str, str]] = []
...
class ParsingResult(BaseModel):
    ...
    raw_experiments: List[dict] = (
        []
    )  # 阶段一原始实验分类列表（含 section_text，供下游全量参考）
...
class CircuitSchema(BaseModel):
    ...
    circuit_metadata: dict = {"filename": "unknown.circ", "has_clock": False}
