...
def resolve_circuit_path(
    initial_path: str | Path | None, workspace_dir: Path
) -> Path | None:
    """
    强化的电路路径解析方案：
    1. 强制转为绝对路径校验
    ...
    # 策略 3: 全局递归搜索文件名
    print(
        f"[PathResolver] 警告: 路径失效 {initial_path}，尝试在项目目录中搜索 {filename}..."
    )
...
async def main():
    ...
    for task in parsing_result.design_tasks:
        ...
        # 3b. DesignAgent 内部已执行子任务验证，这里直接汇总
        all_design_subs.extend(sub_tasks)
