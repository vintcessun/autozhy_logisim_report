...
def make_task() -> TaskRecord:
    ...
    info = info_path.read_text(encoding="utf-8").strip() if info_path.exists() else "测试设计任务"
    return TaskRecord(
...
async def test_decompose_returns_task_list_mock():
    ...
    # 模拟 client.models.generate_content
    mock_client.models.generate_content = AsyncMock(return_value=mock_response)

    agent = DesignAgent(mock_client, mock_config, "gemini-flash")
    task = make_task()

    with patch("src.agents.design_agent.retry_llm_call", return_value=mock_response):
