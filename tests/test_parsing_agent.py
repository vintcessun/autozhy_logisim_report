...
from src.agents.content_parsing import (
    DataDecompressor,
...
def test_categorization_logic():
    ...
    with patch("pathlib.Path.iterdir", return_value=files):
        ...
        assert any(
            "REF_" in f.name and ".circ" in f.name for f in cat["reference_circuits"]
        )
...
async def test_parsing_agent_integrated():
    ...
    try:
        ...
        for i, t in enumerate(result.verification_tasks, 1):
            ...
            print(
                f"        起步电路: {Path(t.source_circ[0]).name if t.source_circ else '无'}"
            )
            print(
                f"        描述: {t.analysis_raw[:120]}{'...' if len(t.analysis_raw) > 120 else ''}"
            )
        ...
        for i, t in enumerate(result.design_tasks, 1):
            ...
            print(
                f"        参考电路: {Path(t.reference_circ).name if t.reference_circ else '无'}"
            )
            print(
                f"        描述: {t.analysis_raw[:120]}{'...' if len(t.analysis_raw) > 120 else ''}"
            )
