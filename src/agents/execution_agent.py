...
class ExecutionAgent:
    ...
    async def generate_and_verify_circuit(
        self, design_spec: str, target_path: Path
    ) -> str:
        """
        根据规格和反馈，在内部闭环直到自检通过。
        返回最终生成的 Python 脚本。
        ...
        history = [
            {
                "role": "user",
                "content": f"根据以下设计规格，编写 python 脚本构建 16 位 CLA 电路：\n{design_spec}",
            }
        ]
        ...
        while attempt < self.max_internal_retries:
            ...
            if not build_success:
                ...
                history.append(
                    {
                        "role": "user",
                        "content": "脚本执行构建失败，请检查语法和 LogicCircuitBuilder 的调用方式。",
                    }
                )
                continue
            ...
            if "SUCCESS" in verify_report:
                ...
                history.append(
                    {
                        "role": "user",
                        "content": f"内部自检未通过！请根据以下真值表错误修正逻辑：\n{verify_report}",
                    }
                )
    ...
    def _try_build(self, script: str, target_path: Path) -> bool:
        ...
        try:
            ...
            # 保存
            new_project = RawProject(
                circuits=[result_circuit],
                main=RawMain(name=result_circuit.name),
                root_attrs=[],
                root_text=[],
            )
