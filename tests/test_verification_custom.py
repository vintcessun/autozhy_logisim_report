...
async def test_verification_case_sandbox():
    ...
    client = genai.Client(
        ...
        http_options={'base_url': config.gemini.base_url}
    )
    ...
    try:
        ...
        if updated_task.status == "finished":
            # 导出文字报告
            (base_dir / "output.txt").write_text(updated_task.analysis_raw, encoding="utf-8")
            ...
            if updated_task.assets:
                ...
                if src_img.exists():
                    ...
                    print(f"[Sandbox] SUCCESS: output.png and output.txt generated in {base_dir}")
                else:
                    print(f"[Sandbox] WARNING: Could not find verified image at {src_img}")
