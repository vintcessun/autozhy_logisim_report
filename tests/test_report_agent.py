...
async def test_report_structure_mock(tmp_path):
    ...
    with patch(
        "src.agents.report_agent.generate_content_with_tools",
        AsyncMock(side_effect=mock_llm_call),
    ):
        # 即使 read_text 返回空文件内容，我们的 mock 也能匹配到某些内容
        # 但为了稳妥，我们 mock Path.read_text 返回包含关键字的内容
        def mock_read_text(self, *args, **kwargs):
            if "intro.txt" in str(self):
                return "实验目的 指导书 {reference_content}"
            if "challenge_split.txt" in str(self):
                return "挑战性 {task_list}"
...
async def test_report_agent_integrated():
    ...
    agent = ReportAgent(
        client, app_config.gemini.model_pro, app_config.gemini.model_flash
    )

    v_tasks = [
        TaskRecord(task_name="验证1", task_type="verification", analysis_raw="分析1")
    ]
    d_tasks = [
        TaskRecord(
            task_id="d1", task_name="设计1", task_type="design", analysis_raw="分析2"
        )
    ]
